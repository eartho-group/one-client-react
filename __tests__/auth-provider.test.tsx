...
describe('EarthoOneProvider', () => {
  ...
  it('should login with a popup', async () => {
    ...
    clientMock.getUser.mockResolvedValue(user);
    act(() => {
      result.current.connectWithPopup();
    });
    expect(result.current.isLoading).toBe(true);
    await waitForNextUpdate();
    expect(result.current.isLoading).toBe(false);
  ...
  it('should call getAccessTokenSilently in the scope of the Eartho client', async () => {
  ...
  it('should call getAccessTokenWithPopup in the scope of the Eartho client', async () => {
